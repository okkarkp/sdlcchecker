...
- Diagnose build failures (per module), edit build config / Dockerfiles, edit pipeline / CI
  ...
  has one.
- For a feature, run the final clean build for the touched module(s) only, then report the
  build gate as GREEN or RED (with the failing output) for the orchestrator's Gate ledger.
